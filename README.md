...
Nginx (:8080)
  ├── /api/*        → PHP-FPM (Symfony)
  └── /geojson/*   → static GeoJSON files (public, cached)
        │  FastCGI
        ▼
PHP container
...
```bash
cp .env.example .env   # already exists as .env — edit if needed
```
...
NEXT_PUBLIC_ENTRYPOINT=http://localhost:8080
```
...
├── docker/
│   ├── nginx/default.conf  Reverse proxy, 600 MB upload limit
│   └── php/
│       ├── Dockerfile      PHP 8.4-FPM + GDAL tools
│       └── entrypoint.sh   Starts worker + PHP-FPM
