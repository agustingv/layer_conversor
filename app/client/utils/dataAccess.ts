...
export const fetch = async <TData>(
  ...
  const normalizedId = id.startsWith("/api/") ? id.slice(4) : id;
  const resp = await isomorphicFetch(ENTRYPOINT + normalizedId, init);
  ...
  const errorMessage = json["title"] ?? resp.statusText;
  const status = json["detail"] || resp.statusText;
